...
export default function CashOut({ projectId, onCashOutCreated = null }) {
  const { auth } = useAuth();
  ...
  // Fetch accounts and projects on mount
  useEffect(() => {
    if (auth?.accessToken) {
      fetchAccounts();
      fetchProjects();
  ...
  // Auto-select project when projectId prop changes
  useEffect(() => {
    if (projectId) {
      setFormData(prev => ({
        ...prev,
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <h1 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">Cash Out Form</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
          ...
          <div>
            <label className="block mb-2 font-semibold text-sm md:text-base">Select Project</label>
            <select
              name="project"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-red-500 text-white py-2 rounded hover:bg-red-600 disabled:bg-gray-400 text-sm md:text-base font-medium"
          >
            {loading ? 'Submitting...' : 'Submit Cash Out'}
          </button>
        </form>
    </div>
  );
